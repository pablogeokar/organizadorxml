...
if (isset($_POST) && isset($_POST['enviar'])) {
	...
	$numeroNfe =				filter_input(INPUT_POST, 'numero_nfe', FILTER_SANITIZE_NUMBER_INT);
	...
	$queryBuscar = "SELECT * FROM nota_fiscal WHERE 1";
...
<html lang="pt-br">
	<head>
		<meta charset="utf-8">
		<title>Pesquisar nota fiscal eletrônica</title>
	...
	<body>
		...
		<div class="container-narrow">
			...
			<form name="pesquisar" id="pesquisar" class="form-inline" method="post">
				...
				<p>
					<input type="text" class="span6" name="numero_nfe" placeholder="Número nfe">
				</p>
			...
			<div class="row-fluid">
				<?php if ((is_object($resultados)) && ($resultados->rowCount() > 0)) : ?>
					...
					<table class="table table-striped">
						...
						<tbody>
							...
							foreach ($resultados as $resultado) {
								print "<tr>";
								print "<td>" . date('d/m/Y', strtotime($resultado['data_emissao_nota'])) . "</td>";
								print "<td>" . $resultado['chave_nota'] . "</td>";
								print "<td>" . $resultado['numero_nota'] . "</td>";
					...
					<hr>
			<?php print "<a class='btn btn-success' href='download.php?tipo=nfe
&dataInicialLancamento=$dataInicialLancamento&dataFinalLancamento=$dataFinalLancamento&cnpjCpfEmitente=$cnpjCpfEmitente
&cnpjCpfDestinatario=$cnpjCpfDestinatario&numeroNfe=$numeroNfe&chaveNfe=$chaveNfe'
