...
$resultados = 0;
if ((isset($_GET)) && ( (isset($_GET['xml_esta_no_sistema'])) || (isset($_GET['chave_nfe'])) )) {
	if (isset($_GET['data_inicial_lancamento'])) $dataInicialLancamento = date('Y-m-d', strtotime($_GET['data_inicial_lancamento']));
	if (isset($_GET['data_final_lancamento'])) $dataFinalLancamento = date('Y-m-d', strtotime($_GET['data_final_lancamento']));
	$cnpjCpfEmitente =		filter_input(INPUT_GET, 'cnpj_cpf_emitente', FILTER_SANITIZE_NUMBER_INT);
	$chaveNfe =			filter_input(INPUT_GET, 'chave_nfe', FILTER_SANITIZE_NUMBER_INT);
	$valorNfe =			filter_input(INPUT_GET, 'valor', FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);
	$xmlEstaNoSistema =		filter_input(INPUT_GET, 'xml_esta_no_sistema', FILTER_SANITIZE_STRING);
	$campos = array();
	...
	if (!empty($xmlEstaNoSistema)) {
		if (strtolower($xmlEstaNoSistema) == 'sim') $xmlEstaNoSistema = "true";
		else	$xmlEstaNoSistema = 'false';
	}
	if (isset($dataInicialLancamento) && !empty($dataInicialLancamento)) $campos += array("data_emissao >='" => $dataInicialLancamento . "'");
	if (isset($dataFinalLancamento) && !empty($dataFinalLancamento))	$campos += array("data_emissao <='" => $dataFinalLancamento . "'");
	if (isset($cnpjCpfEmitente) && !empty($cnpjCpfEmitente)) $campos += array('cnpj_cpf_emitente =' => $cnpjCpfEmitente);
	if (isset($chaveNfe) && !empty($chaveNfe)) $campos += array('chave =' => $chaveNfe);
	if (isset($valorNfe) && !empty($valorNfe)) $campos += array("valor LIKE" => "'" . $valorNfe . "'");
	if (isset($xmlEstaNoSistema) && !empty($xmlEstaNoSistema)) $campos += array('xml_esta_no_sistema =' => $xmlEstaNoSistema);
	$queryBuscar = "SELECT * FROM nota_fiscal_destinada WHERE 1";
...
<html lang="pt-br">
	<head>
		<meta charset="utf-8">
		<title>Pesquisar nota fiscal eletrônica emitida para esta empresa</title>
	...
	<body>
		...
		<div class="container-narrow">
			...
			<form name="pesquisar" id="pesquisar" class="form-inline" method="get">
				<legend>Pesquisar nota fiscal emitida para esta empresa</legend>
				...
				<p>
					...
					<input type="text" class="span3" name="valor" placeholder="Valor da nota fiscal">
				</p>
					...
					<table class="table table-striped">
						...
						<tbody>
							...
							foreach ($resultados as $resultado) {
								print "<tr>";
								print "<td>" . date('d/m/Y', strtotime($resultado['data_emissao'])) . "</td>";
								print "<td>" . $tiposRegistro[$resultado['tipo_registro']] . "</td>";
								print "<td>" . substr($resultado['chave'], 0, 23) . " " . substr($resultado['chave'], 23) . "</td>";
								print "<td>" . $resultado['nome_emitente'] . "</td>";
								print "<td>" . $tiposOperacao[$resultado['tipo_operacao']] . "</td>";
								print "<td>" . $resultado['valor'] . "</td>";
								print "<td>" . $situacoesNfe[$resultado['situacao_nota']] . "</td>";
								print "<td>" . $situacoesManifesto[$resultado['situacao_manifesto']] . "</td>";
								...
								print "<td><a alt='Download' title='Download' href=download.php?tipo=nfe&id=".$resultado['id']."><i class='icon-download-alt'</i></a>";
								print "<td><a alt='Imprimir' title='Imprimir' target='_blank' href=imprimir.php?tipo=nfe&id=".$resultado['id']."><i class='icon-print'</i></a>";
								 */print "</tr>";
							}
							?>
						</tbody>
